...
VERSIONS_FILE=${VERSIONS_FILE:-"rag_sources/spring_boot_versions.txt"}
MAX_FILES=${MAX_FILES:-""}
FORCE=${FORCE:-false}
...
log_file="$LOG_DIR/ingest-spring-boot-${timestamp}.log"
...
PAYLOAD
  ...
  echo "Ingestion Spring Boot $version" | tee -a "$log_file"
  ...
  curl -sS -X POST "$RAG_BASE_URL/api/rag/ingest/spring-boot-source" \
